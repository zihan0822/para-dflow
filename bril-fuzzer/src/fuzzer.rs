use crate::dist::{ArithInst, BoolInst, Context, Sample};
use bril_rs::program::*;
...
pub fn generate_bril_program(num_fns: usize) -> Program {
    Program {
        functions: (0..num_fns).map(|_| generate_code_blk(8)).collect(),
    }
}

fn generate_code_blk(num_instrs: usize) -> Function {
...
fn parse_dest_and_ty(instr: &Instruction) -> (String, Type) {
    match instr {
        ...
        Instruction::Constant {dest, const_type, ..} => (dest.clone(), const_type.clone()),
        _ => unreachable!()
    }
}
